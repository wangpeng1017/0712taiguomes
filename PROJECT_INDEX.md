...
泰国工厂汽车电子结构件（注塑+冲压，线束预留）轻量 MES 一期 Demo：工单驱动生产，批次支撑追溯，模具关联寿命，日报自动汇总。当前状态：**核心功能本地开发+浏览器实地验证已完成（含模具超寿命二次确认等关键规则），尚未部署到阿里云测试环境**。
...
部署：🔴 尚未确定（阿里云测试服务，细节待王老师提供），见 `docs/SHIP-PROFILE.md`。
